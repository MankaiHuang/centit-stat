#!/bin/sh
DEPLOYMENT=centit-stat
MOUDLE=centit-stat-web
TIME='date +%Y%m%d%H%M'
GIT_REVISION='git log -1 --pretty=format:"%h"'
IMAGE_NAME=172.29.0.13:8082/${DEPLOYMENT}:${TIME}_${GIT_REVISION}
#编译
mvn -U -pl ${MODULE} -am clean package -DskipTests=true
#写入dockerfile文件
...
#运行
docker run -d --network host -p 13000:8080 --name ${DEPLOYMENT} ${IMAGE_NAME}
