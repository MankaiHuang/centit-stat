define(['app', 'echarts'], function(model, echarts) {
  ...
  model.factory("TwodimenForm", ['Restangular', function (Restangular) {
    var twodimen = Restangular.all(RestangularContextPath + "service/stat/twodimenform/");
    return twodimen;
  }]);
  model.directive('statChart', function () {
    return {
      restrict: 'E',
      templateUrl: ContextPath + 'modules/stat/template/chart.html',
      replace: true,
  ...
  model.directive('chartPanel', function () {
    return {
      restrict: 'E',
      templateUrl: ContextPath + 'modules/stat/template/chartspanel.html',
      replace: true,
      scope: {
        chartcols: '=chartcols'
      },
  ...
  model.controller('TwoDimenFormController', ['$scope', 'TwodimenForm', '$location', function ($scope, TwodimenForm, $location) {
    ...
    //页面大小切换选项
    $scope.pageSizeChoose = [{key: 10, value: 10}, {key: 15, value: 15}, {key: 20, value: 20}, {
      key: 30,
      value: 30
    }, {key: 50, value: 50}, {key: 100, value: 100}, {key: 500, value: 500}];
